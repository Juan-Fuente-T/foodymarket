...
public class AuthController {
    ...
    public ResponseEntity<AuthResponseDto> register(@Valid @RequestBody final UserRequestDto registerDto){
        final AuthResponseDto response = authService.register(registerDto);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/login")
    public ResponseEntity<AuthResponseDto> authenticate(@Valid @RequestBody final LoginRequestDto request){
