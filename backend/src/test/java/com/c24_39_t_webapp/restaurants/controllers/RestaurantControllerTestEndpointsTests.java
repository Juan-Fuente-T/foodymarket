...
/**
 * Test para los endpoints de prueba del RestaurantController
 ...
 * Estos endpoints no requieren autenticación (están configurados como públicos en SecurityConfig)
...
@Slf4j
@DisplayName("RestaurantController - Test Endpoints (testMethod, testPostMethod)")
public class RestaurantControllerTestEndpointsTests {
    /**
    ...
    private static final String TEST_NAME = "MiRestaurante";
    ...
    /**
     * Mock del servicio de restaurantes (no se usa en endpoints de prueba, pero Spring lo requiere)
     */
    @MockitoBean
    ...
    @Nested
    @DisplayName("GET /api/restaurant/testMethod")
    class GetTestEndpointTests {
         ...
         */
        @Test
        @DisplayName("GET /api/restaurant/testMethod - Retorna 200 OK con mensaje de prueba")
        void whenCallTestGetMethod_thenReturnsOkWithTestMessage() throws Exception {
            ...
            // Act & Assert
            mockMvc.perform(get(RESTAURANT_ENDPOINT + TEST_GET_ENDPOINT)
                            .with(user(RESTAURANT_EMAIL).roles("RESTAURANTE")))
//                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.TEXT_PLAIN_VALUE + ";charset=UTF-8"))
                    .andExpect(content().string(containsString("El metodo GET del controller de Restaurants funciona ok!")));

            // Verify que el servicio NO fue llamado
            verify(restaurantService, never()).findAll();
            verify(restaurantService, never()).findById(any());
        }
         ...
         */
        @Test
        @DisplayName("GET /api/restaurant/testMethod - Valida el mensaje exacto retornado")
        void whenCallTestGetMethod_thenReturnsExactMessage() throws Exception {
            // Act & Assert
            mockMvc.perform(get(RESTAURANT_ENDPOINT + TEST_GET_ENDPOINT)
                            .with(user(RESTAURANT_EMAIL).roles("RESTAURANTE")))
                    .andExpect(status().isOk())
                    .andExpect(content().string("El metodo GET del controller de Restaurants funciona ok!"));
        }
    }
    ...
    @Nested
    @DisplayName("POST /api/restaurant/testPostMethod")
    class PostTestEndpointTests {
         ...
         */
        @Test
        @DisplayName("POST /api/restaurant/testPostMethod - Retorna 200 OK con mensaje incluyendo el parámetro")
        void whenCallTestPostMethod_thenReturnsOkWithParameterIncluded() throws Exception {
            ...
            // Act & Assert
            mockMvc.perform(post(RESTAURANT_ENDPOINT + TEST_POST_ENDPOINT)
                    ...
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.TEXT_PLAIN_VALUE + ";charset=UTF-8"))
                    .andExpect(content().string(containsString("El metodo POST del controller de Restaurants funciona ok, " + testNameParameter + "!")));
         ...
         */
        @Test
        @DisplayName("POST /api/restaurant/testPostMethod - Valida el mensaje exacto con parámetro")
        void whenCallTestPostMethod_thenReturnsExactMessageWithParameter() throws Exception {
            // Arrange
            String testNameParameter = "AtlanticoTest";

            // Act & Assert
            mockMvc.perform(post(RESTAURANT_ENDPOINT + TEST_POST_ENDPOINT)
                            .contentType(MediaType.APPLICATION_JSON)
//                            .content(objectMapper.writeValueAsString(testNameParameter))
                            .content(testNameParameter)
                            .with(user(RESTAURANT_EMAIL).roles("RESTAURANTE"))
                            .with(csrf()))
                    .andExpect(status().isOk())
                    .andExpect(content().string("El metodo POST del controller de Restaurants funciona ok, " + testNameParameter + "!"));
        }

        /**
         * Test que verifica que el endpoint testPostMethod maneja correctamente
         * un parámetro con espacios en blanco
         * Arrange: Prepara un nombre con espacios
         * Act & Assert: Realiza la petición POST con espacios
         *
         * @throws Exception
         */
        @Test
        @DisplayName("POST /api/restaurant/testPostMethod - Maneja correctamente parámetros con espacios")
        void whenCallTestPostMethodWithSpaces_thenReturnsMessageWithSpaces() throws Exception {
            // Arrange
            String parameterWithSpaces = "Mi Restaurante Favorito";

            // Act & Assert
            mockMvc.perform(post(RESTAURANT_ENDPOINT + TEST_POST_ENDPOINT)
                            .contentType(MediaType.APPLICATION_JSON)
//                            .content(objectMapper.writeValueAsString(parameterWithSpaces))
                            .content(parameterWithSpaces)
                            .with(user(RESTAURANT_EMAIL).roles("RESTAURANTE"))
                            .with(csrf()))
                    .andExpect(status().isOk())
                    .andExpect(content().string("El metodo POST del controller de Restaurants funciona ok, " + parameterWithSpaces + "!"));
        }
    }
}